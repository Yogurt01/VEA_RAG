...
def run(args):
    ...
    def format_counts(counter):
        return ', '.join(f"Label {k}: {v}" for k, v in sorted(counter.items()))

    print(f"Train set: {len(train_ids)} videos ({format_counts(train_counts)})")
    print(f"Val set  : {len(val_ids)} videos ({format_counts(val_counts)})")
    print(f"Test set : {len(test_ids)} videos ({format_counts(test_counts)})")
