...
def getTokens(sent):
    ...
    for token in tokens:
        pos = getText(token.getElementsByTagName('POS')[0].childNodes)
        pos = pos.encode("ascii", "ignore").decode("utf-8")
        word = getText(token.getElementsByTagName('word')[0].childNodes)
        word = word.encode("ascii", "ignore").decode("utf-8")
        lemma = getText(token.getElementsByTagName('lemma')[0].childNodes)
        lemma = lemma.encode("ascii", "ignore").decode("utf-8")
        try:
            ner = getText(token.getElementsByTagName('NER')[0].childNodes)
            ner = ner.encode("ascii", "ignore").decode("utf-8")
        except IndexError:
            ner = None
        idx = int(token.attributes['id'].value)
        token = TokenElem(idx, word, lemma, pos, ner)
...
def combineparse2sent(sent, parse):
    """ Combine constituent parse into sent
    """
    parse = parse.split()
    tokenlist = [token.word for token in sent.tokenlist]
    parselist, tidx = [""] * len(tokenlist), 0
    while parse:
        ...
        parselist[tidx] += (" " + item)
        partialparse = parselist[tidx].replace(' ', '')
        partialparse = partialparse.encode("ascii", "ignore").decode("utf-8")
        word = tokenlist[tidx].replace(' ', '')
        if (word + ')') in partialparse:
            tidx += 1
    # Attach to sent
...
def writer(sentlist, fconll):
    with open(fconll, 'w', encoding='utf-8') as fout:
        for sent in sentlist:
            for token in sent.tokenlist:
                line = (
                    str(sent.idx) + '\t' + str(token.idx) + '\t' + token.word + '\t' +
                    token.lemma + '\t' + str(token.pos) + '\t' + str(token.deptype) + '\t' +
                    str(token.headidx) + '\t' + str(token.nertype) + '\t' +
                    str(token.partialparse) + '\n'
                )
                fout.write(line)
            fout.write('\n')
...
if __name__ == '__main__':
    sentlist, constlist = reader('/Users/mohdtahaabbas/Documents/python program files/example1.txt.xml')
    sentlist = combine(sentlist, constlist)
    writer(sentlist, 'test.conll')
