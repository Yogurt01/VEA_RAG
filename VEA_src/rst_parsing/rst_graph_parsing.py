...
class RSTCaptionGraphBuilder:
    ...
    def _video_dirs(self) -> Iterable[Path]:
        dirs = [p for p in self.root_dir.iterdir() if p.is_dir()]
        dirs = sorted(dirs)
        if self.limit_videos > 0:
            dirs = dirs[:self.limit_videos]
        return dirs
...
def main() -> int:
    ...
    parser.add_argument("--limit_videos", type=int, default=0, help="Limit number of videos to process (0 = all)")
