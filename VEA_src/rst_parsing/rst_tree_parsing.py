...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RST Tree parsing for video captions")

    parser.add_argument("--base_model_name", type=str, default="/models/Llama-2-7b-hf")
    parser.add_argument('--adapter_base_path', type=str, default='/models/adapters')
    ...
    parser.add_argument("--dataset_file", type=str, default="/EnTube/edu_dataset.json", required=True, help="Path to JSON file: list[{doc_id, edu_strings, video_dir?}]")
    parser.add_argument("--limit_videos", type=int, default=0, help="Limit the number of docs processed in this run, after filtering out those that already have rst_tree.tree (0 = no limit).")
