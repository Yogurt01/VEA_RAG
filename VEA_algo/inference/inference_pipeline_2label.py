...
--------------------------------------------------------------------------------------------------
Usage:
    python inference_pipeline_2label.py --evidence_mode full \
...
EVIDENCE_MODE_CHOICES = ["none", "full"]
...
# ==========================================
# 5. DISCOURSE PATH SERIALIZATION (DPS)
# ==========================================
...
def load_candidate_video_data(video_id: str, data_root: Path, cache: dict) -> dict:
    """
    Load (và cache trong 1 lần gọi retrieve) topology vector + RAW caption (KHÔNG
    DPS-annotate) của 1 video training — dùng cho Chặng 2 (Topology Re-ranking).
    Lưu ý: caption hiển thị trong REFERENCE block giữ nguyên bản gốc, không gắn
    thẻ [Discourse: ...] — thẻ này chỉ áp dụng cho INPUT VIDEO CONTENT.
    """
...
def retrieve_discourse_evidence(
    ...
    """
    Chặng 1 (Dense Vector Search): search MỘT LẦN DUY NHẤT trên collection
    scene-level bằng `query_vec` — đây chính là V_video của video test, đã
    được tính sẵn bởi Cross-Modal Self-Querying (mean-pooling caption → text
    query → temperature-softmax re-weight visual scene embeddings). KHÔNG còn
    lặp qua từng scene như phiên bản trước — toàn bộ video giờ được đại diện
    bởi đúng 1 vector truy vấn.

    Chặng 2 (Topology/Nuclearity Re-ranking): với các scene-hit trả về từ
    Chặng 1, blend dense similarity (từ V_video) với cosine similarity giữa
    topology vector (Nuclearity Scoring) của video test và từng video ứng viên.

    Trả về ranked_top: list[(video_id, cand_dict)] đã dedupe theo video (giữ
    hit có final blended score cao nhất mỗi video), sort giảm dần theo score.
    """
    ...
    for h in hits:
        ...
        if vid not in best_per_video or final_score > best_per_video[vid]["score"]:
            ...
            matched_caption = re.sub(r'\s*\[Discourse:.*?\]', '', matched_caption)
            try:
...
# ==========================================
# 6. CONTEXT BUILDER — REFERENCE (dedupe theo video, không mớm nhãn)
# ==========================================
...
# ==========================================
# 8. ENSEMBLE — tie-break theo CONFIDENCE
# ==========================================

def compute_ensemble_label(llm_pred, ref_lean, ref_conf):
    """
    Vote đa số giữa 2 tín hiệu (LLM prediction + reference retrieval lean).
    Tie-break: nếu bất đồng, ưu tiên ref_lean khi confidence 'strong',
    ngược lại giữ nguyên llm_pred.
    """
...
def main(args: argparse.Namespace) -> None:
    ...
    need_reference = evidence_mode == "full"
    ...
    if need_reference:
        ...
        if not collection_name:
            raise ValueError(
                ...
                "MILVUS_COLLECTION_NAME env var (required for evidence_mode='full')."
            )

        if not args.video_reps_dir:
            raise ValueError(
                "--video_reps_dir is required when evidence_mode='full' "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="VideoRAG Inference Pipeline — Cross-Modal Self-Querying (V_video) + "
                    "Nuclearity-Weighted RAG / Discourse Path Serialization (DPS), single merged "
                    "reference-retrieval channel. Ablation: none | full.",
    ...
    parser.add_argument("--evidence_mode",     type=str, default="full", choices=EVIDENCE_MODE_CHOICES,
                        help="'none': không dùng reference library. 'full': Cross-Modal Self-Querying (V_video) "
                             "→ Milvus dense search → Nuclearity-Weighted topology re-rank → DPS-annotated evidence.")
    parser.add_argument("--collection_name",   type=str, default=None,
                        help="Milvus collection scene-level. Mặc định env MILVUS_COLLECTION_NAME.")
    parser.add_argument("--video_reps_dir",    type=str, default=None,
                        help="Dir chứa video_representations.pt (V_video từ Cross-Modal Self-Querying). "
                             "Bắt buộc khi evidence_mode='full'.")
    ...
    # --- Reference retrieval hyperparameters (Cross-Modal Self-Querying → NW-RAG → DPS) ---
    parser.add_argument("--top_k", type=int, default=5,
                        help="Số VIDEO khác nhau (đã dedupe) lấy làm evidence tham chiếu.")
    parser.add_argument("--search_limit", type=int, default=50,
                        help="Số scene-candidate thô lấy từ Milvus trong MỘT LẦN search bằng V_video "
                             "(nên >> top_k, vì sẽ bị dedupe theo video).")
