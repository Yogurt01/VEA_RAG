"""
milvus_compute_video_query.py
--------------------------------
Tính toán vector đại diện V_video cho từng video test bằng Cross-Modal Self-Querying
với Qwen3-VL-Embedding-2B — đây là bước QUERY-SIDE của kênh Content Similarity
(dùng cùng với milvus_upload_scene_index.py ở phía INDEX-SIDE).

Usage:
    python milvus_compute_video_query.py \
...
đồng bộ với convention scene_embeddings.pt của project):
    - video_representations.pt        : {folder_name: torch.Tensor(2048,)}
    - video_representations_meta.json : {metadata, videos: [{folder_name, scene_captions, temperature}]}
"""
...
def load_scene_captions(folder_name: str, data_root: Path) -> List[str]:
    """
    Đọc captions từ segments.json theo đúng thứ tự scene_ids.
    Trả về list các caption (string), nếu thiếu thì thay bằng chuỗi rỗng.
    """
...
def cross_modal_self_query(
    ...
    """
    Tạo vector đại diện V_video ∈ R^2048 bằng query-weighted aggregation.
    ...
    V_norm = F.normalize(V, p=2, dim=1)   # (T, 2048)
    q_norm = F.normalize(q, p=2, dim=0)   # (2048,)
...
def main(args: argparse.Namespace) -> None:
    ...
    for idx, folder in enumerate(valid_folders, 1):
        ...
        results.append({
            "folder_name": folder,
            "scene_captions": scene_captions,
            "v_video": v_video.cpu().float(),   # giữ tensor, tách khỏi JSON ở bước save
            "temperature": args.temperature,
        })
    ...
    # 4. Save results — TÁCH embeddings (binary, nhanh) khỏi metadata (JSON, dễ đọc)
