...
def _aggregate_scores(loader, model, class_idx, task: Task = Task.GRAPH_CLASSIFICATION,
    ...
    for data in iter(loader):
        if task == Task.GRAPH_CLASSIFICATION:
            scores = model(data.x.to(device),
                           data.edge_index.to(device),
                           data.edge_type.to(device),
                           data.batch.to(device)).detach().cpu()
...
def mc_l_shapley(model: torch.nn.Module, graph: Data, subgraph: Set[int], t: int, num_layers: int,
    ...
    if task == Task.GRAPH_CLASSIFICATION or task == Task.NODE_CLASSIFICATION:
        scores = model(x=graph.x.to(device), edge_index=graph.edge_index.to(device), edge_type=graph.edge_type.to(device),
                       batch=torch.zeros(graph.num_nodes).long().to(device)).detach().cpu()
    else:
        x1 = torch.tensor([nodes_to_keep[0]]).long().to(device)  # node 1 of edge
        x2 = torch.tensor([nodes_to_keep[1]]).long().to(device)  # node 2 of edge
        scores = model(x=graph.x.to(device), edge_index=graph.edge_index.to(device), edge_type=graph.edge_type.to(device),
                       x1=x1, x2=x2, ptr=None).detach().cpu()
    ...
    for i in range(t):
        ...
        masked_x_include = graph.x * torch.tensor(include_mask, device=graph.x.device).unsqueeze(1)  # zero padding
        include_data = Data(x=masked_x_include.float(),
        ...
        masked_x_exclude = graph.x * torch.tensor(exclude_mask, device=graph.x.device).unsqueeze(1)  # zero padding
        exclude_data = Data(x=masked_x_exclude.float(),
                            edge_index=graph.edge_index,
                            edge_type=graph.edge_type)
