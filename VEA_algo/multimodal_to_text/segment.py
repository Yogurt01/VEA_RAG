...
def segment_with_stt_timestamp(video_dir, starts, ends):
    """
    Ghép transcript (audio.json) vào ranh giới cảnh (starts/ends).
    Trả về list[dict] với keys: start, end, text, speaker  (mỗi phần tử là 1 scene).
    Nếu không có audio.json → mỗi scene có text/speaker rỗng.
    """
...
def scene_detection(video_path, sensitive_detector=None):
    """
    Chạy 3 detector theo thứ tự, dừng ngay khi scenes > 3.
    Trả về (starts, ends) hoặc (None, None) nếu không detect được.
    """
...
def _motion_boundaries(video_path, sample_fps=5.0, smooth_window=5,
    ...
    Phù hợp với video tĩnh / talking-head / slow-motion.
    """
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        return []
...
def _stt_gap_boundaries(video_dir, min_gap_sec=1.5):
    """
    Khoảng hở giữa 2 STT segment → boundary tại midpoint.
    """
    json_path = Path(video_dir) / 'audio.json'
...
def _merge_and_ensure_min_nodes(all_boundaries, duration, min_nodes=3,
    ...
    """
    1. Sort + loại boundary quá gần đầu/cuối
    2. Dedup: bỏ boundary gần nhau < merge_gap
    3. Nếu #nodes < min_nodes → thêm temporal split để đủ
    Trả về (starts, ends).
    """
    ...
    if n_nodes < min_nodes:
        # Chèn thêm boundary phân bổ đều, tránh trùng
        ...
        # Hard fallback: vẫn thiếu (video quá ngắn)
        if n_nodes < min_nodes:
            step    = duration / min_nodes
...
def fallback_boundaries(video_path, video_dir, min_nodes=3, merge_gap=1.0):
    """
    Gọi tất cả fallback signal, merge lại, trả về (starts, ends).
    """
